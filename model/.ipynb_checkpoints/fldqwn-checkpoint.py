...
class WN_SubBlock(nn.Module):  # 一个Block，包含一个低通滤波器和一个高通滤波器
    ...
    def forward(self, input_data):
        input_data = pad(input_data, self.padsize, horizontal=self.horizontal)
    ...
    def forward(self, x):
        ...
        if(self.regu_approx + self.regu_details != 0.0):
            if self.regu_details:
                ...
                rd += self.regu_details * lh.abs().mean()
                rd += self.regu_details * hh.abs().mean()
            if self.regu_approx:
